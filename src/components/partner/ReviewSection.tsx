        ...
        <div className="flex flex-col items-center justify-center min-w-[150px]">
          ...
          <div className="flex gap-1 mb-1">
            {[1, 2, 3, 4, 5].map((star) => (
              <Star
                key={star}
                className={`w-5 h-5 ${star <= Math.round(summary.avg_rating) ? 'fill-yellow-400 text-yellow-400' : 'fill-gray-200 text-gray-200'}`}
              />
            ))}
          </div>
          <div className="text-sm text-gray-500">{summary.total_reviews} Ulasan</div>
        </div>
        
        <div className="flex-1 space-y-2">
          {[
            { stars: 5, count: summary.count_5 },
            { stars: 4, count: summary.count_4 },
            { stars: 3, count: summary.count_3 },
            { stars: 2, count: summary.count_2 },
            { stars: 1, count: summary.count_1 },
          ].map((item) => (
          ...
          <div key={review.id} className="flex gap-4">
            ...
            <div className="flex-1">
              <div className="flex justify-between items-start mb-1">
                <div>
                  <h4 className="font-medium text-gray-900 text-sm sm:text-base">{review.customer_name}</h4>
