...
export default function SortBar({ onOpenFilter, sort, onSortChange }: SortBarProps) {
  const getButtonClass = (value: string) => {
    ...
    return isActive
      ...
      : "bg-white border border-[#e5e2e1] text-[#1c1b1b] text-[12px] md:text-[14px] py-1 px-2.5 md:py-[7px] md:px-[17px] rounded-md hover:border-brand-red whitespace-nowrap transition-all";
  };
  return (
    <div className="flex items-center gap-1.5 md:gap-4 bg-[#f6f3f2] border border-[#e5e2e1] rounded-lg p-1.5 md:p-[9px] w-full overflow-x-auto scrollbar-hide">

      {/* Mobile Filter Button */}
      <button
        onClick={onOpenFilter}
        className="md:hidden flex items-center gap-1 bg-white border border-[#e5e2e1] text-[#1c1b1b] text-[12px] md:text-[14px] font-medium py-1 px-2 md:py-[7px] md:px-3 rounded-md whitespace-nowrap transition-all shadow-sm"
      >
