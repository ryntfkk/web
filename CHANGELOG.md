...
- **Dua jalur kontak, dipisah menurut siapa yang boleh membaca.** Lamaran peran →
  `teamContactHref()` → `profile.support_email` dari `/config` (publik, bisa diganti dari
  panel admin). Percakapan investor & permintaan deck → `founderContactHref()` →
  konstanta `FOUNDER_EMAIL` (jalur langsung founder, sementara, ganti = redeploy).
  Jalur tim sengaja jatuh ke `/help` bila config gagal . BUKAN ke alamat founder.
