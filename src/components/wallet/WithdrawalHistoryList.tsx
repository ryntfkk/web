 ...
 * penarikan hanya berupa satu baris DEBIT: pengguna tidak bisa melihat penarikan
 * mana yang ditolak, kapan diproses, ke rekening mana, dan mengapa gagal.
 * Semua itu hidup di tabel `withdrawals`.
 *
 * BADAN halaman saja . tanpa header/kontainer. `GET /wallet/withdrawals` tidak
 * pernah punya gerbang peran (lihat catatan A04-T1 di `wallet/handler.go`), jadi
 * daftar yang sama sah untuk mitra maupun pelanggan; yang berbeda hanya
 * kerangkanya. Menyalin berkas ini ke sisi pelanggan akan membuat dua salinan
 * aturan status yang pasti menyimpang.
 */
...
interface Props {
  ...
  enabled: boolean;
  /**
   * Kelas grid daftar. Mode mitra memakai dua kolom di `lg` (ruang desktopnya
   * lebar); halaman pelanggan hidup di kolom yang lebih sempit.
   */
  listClassName?: string;
}

export default function WithdrawalHistoryList({ enabled, listClassName = 'grid gap-3' }: Props) {
